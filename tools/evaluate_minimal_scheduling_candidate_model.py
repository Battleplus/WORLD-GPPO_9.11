...
def metrics(model: CandidateRegretNet, rows: list[dict[str, object]], device: torch.device, timing: bool = False) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    def aggregate(subset: list[dict[str, object]]) -> dict[str, object]:
        ...
        return {"instances": len(subset), "mean_normalized_regret": float(np.mean(model_reg)), "mean_raw_regret": float(np.mean([x["selected_raw_regret"] for x in subset])), "mean_regret_reduction_vs_greedy": float(np.mean(diffs)), "optimal_first_action_rate": float(np.mean([x["selected_is_optimal"] for x in subset])), "greedy_mean_normalized_regret": float(np.mean([x["greedy_normalized_regret"] for x in subset])), "greedy_mean_raw_regret": float(np.mean([x["greedy_raw_regret"] for x in subset])), "greedy_optimal_first_action_rate": float(np.mean([x["greedy_is_optimal"] for x in subset])), "prediction_mae_all_candidates": float(np.mean([x["prediction_mae_all_candidates"] for x in subset])), "non_tie_pair_accuracy": float(sum(x["non_tie_pair_correct"] for x in subset) / sum(x["non_tie_pairs"] for x in subset)) if sum(x["non_tie_pairs"] for x in subset) else None}
...
def main() -> int:
    ...
    final = {"schema": "m10-minimal-scheduling-candidate-regret-test-results-v1", "data": str(args.data.resolve()), "data_sha256": sha256(args.data), "test_instances": len(rows), "rule_baseline": {"overall": {"instances": len(rows), "mean_normalized_regret": float(np.mean([x["greedy_normalized_regret"] for x in per_seed[0]])), "mean_raw_regret": float(np.mean([x["greedy_raw_regret"] for x in per_seed[0]])), "optimal_first_action_rate": float(np.mean([x["greedy_is_optimal"] for x in per_seed[0]]))}, "by_group": {g: {"instances": sum(x["group"] == g for x in per_seed[0]), "mean_normalized_regret": float(np.mean([x["greedy_normalized_regret"] for x in per_seed[0] if x["group"] == g])), "mean_raw_regret": float(np.mean([x["greedy_raw_regret"] for x in per_seed[0] if x["group"] == g])), "optimal_first_action_rate": float(np.mean([x["greedy_is_optimal"] for x in per_seed[0] if x["group"] == g]))} for g in ("homogeneous", "heterogeneous")}, "note": "one shared rule result; not copied into three seeds"}, "exact_reference": {"overall": {"instances": len(rows), "mean_normalized_regret": 0.0, "mean_raw_regret": 0.0, "optimal_first_action_rate": 1.0}, "note": "exact Q* continuation reference; not a deployable policy"}, "per_seed": seed_results, "bootstrap": bootstrap(per_seed, 10000, 20260915), "selection_protocol": "minimum predicted normalized regret; ties follow frozen earliest-finish task-ID/UAV-ID rule", "scope": "first action only with exact optimal continuation; not end-to-end scheduling"}
    dump(args.out, final)
